...
double normal_distribution(double mean, double stddev) {
    ...
    double u1, u2;
    do {
        u1 = drand_custom(0.0, 1.0);
        u2 = drand_custom(0.0, 1.0);
    } while (u1 <= DBL_MIN);
...
void generate_random_particles(Particle *p, Space space, int count, float galaxy_diameter, float galaxy_thickness)
{
    double a = 1.0;
...
double compute_distance_squared(Particle *p, double x, double y, double z)
{
    double dx = p->x - x;
    double dy = p->y - y;
    double dz = p->z - z;
